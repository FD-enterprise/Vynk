...
export const roomJoinLockSchema = z.object({ roomId: roomIdSchema, locked: z.boolean() });
export const roomParticipantActionSchema = z.object({ roomId: roomIdSchema, participantId: z.string().min(1).max(MAX_PEER_ID_LENGTH).regex(/^[A-Za-z0-9_-]+$/) });
export const roomParticipantMuteSchema = roomParticipantActionSchema.extend({ muted: z.boolean() });
